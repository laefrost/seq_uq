...
def load_excel(train_path: str, val_path: str):
    ...
    # FIX: only compute class weights from train; val weights are unused
...
def main():
    ...
    num_epochs = 2
    # FIX: reduced per-device batch size + gradient accumulation for stability
    # effective batch size = 8 * 8 = 64 (same as before, but won't OOM)
    train_bs = 8
    eval_bs = 16
    grad_accum_steps = 8
    # FIX: lower learning rate for better convergence with pretrained model
    lr = 5e-5
    max_length = 512
    ...
    # FIX: explicit DeBERTa-specific LoRA targets instead of broad "dense" match
    targets = infer_lora_targets(model)
    if any(t in targets for t in ["query_proj", "key_proj", "value_proj"]):
        # DeBERTa attention projections — more precise than including "dense"
        target_modules = ["query_proj", "key_proj", "value_proj", "pos_proj", "pos_q_proj"]
        # Filter to only modules that actually exist in this model
    ...
    lora_config = LoraConfig(
        ...
        lora_alpha=32,
        lora_dropout=0.05,
        bias="none",  # FIX: explicit bias setting
        target_modules=target_modules,
    )
    model = get_peft_model(model, lora_config)
    model.print_trainable_parameters()

    # FIX: add F1-macro alongside accuracy; use F1 as best model metric
