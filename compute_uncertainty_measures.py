...
from utils.subsequences import generate_subsequences, generate_word_subsequences
from sklearn.metrics.pairwise import cosine_similarity
from utils.utils import get_parser, save, setup_logger, load
import numpy as np
...
def se_pipe_across_tokens(question, seq_tokens, ellm, mode = 'adapted'): 
    """
    Computes semantic entropy (SE) across tokens.

    Clusters the alternative token subsequences semantically, then computes
    both original and claim conditional SE across steps.

    Parameters
    ----------
    question : str
        The question used as context for semantic clustering.
    seq_tokens : list of dict
        Per-step token subsequence data.
    ellm : LLM or NLI
        Entailment model used to assign semantic cluster IDs.
    mode : str, optional
        Clustering mode passed to generate_semantic_subsequence_ids. Default is 'adapted'.

    Returns
    -------
    entropies_to : list of float
        Original semantic entropy at each decoding step.
    entropies_to_weighted : list of float
        Claim conditional semantic entropy at each decoding step.
    """
    ...
    entropies_to = compute_se_across_subsequences(cluster_ids_across_steps=cluster_ids_across_steps, probs = probs)  
    entropies_to_weighted = compute_se_across_subsequences(cluster_ids_across_steps=cluster_ids_across_steps, probs = probs, topics = topic_ids_across_steps)    
...
def se_pipe_across_words(question, seq_words, ellm, mode = 'adapted'): 
    """
    Computes semantic entropy (SE) across words.
    Parameters and return values equal to se_pipe_across_tokens()
    """  
...
def uq_pipe_across_tokens(seq_tokens, emb_model, question, gen_ids, tokenizer_llm, mode = 'sampling', task_type = 'qa'):
    """
    Computes kernel-based Von Neumann Entropy (VNE) uncertainty across tokens.
    
    Parameters
    ----------
    seq_tokens : list of dict
        Per-step token subsequence data, including 'alternative_sequence_decoded',
        'alternative_tokens_str', and 'alternative_token_probs'.
    emb_model : SentenceTransformer
        Embedding model used to encode sequences and tokens.
    question : str
        Question prepended to sequences before embedding.
    gen_ids : list of int
        Token IDs of the full generated sequence (unused directly, passed for context).
    tokenizer_llm : transformers.PreTrainedTokenizer
        Tokenizer of the generation LLM (unused directly, passed for context).
    mode : str, optional
        VNE normalization mode ('sampling' or 'prob'). Default is 'sampling'.
    task_type : str, optional
        If not 'qa', embeds only the last sentence of each alternative. Default is 'qa'.

    Returns
    -------
    vnes : list of float
        VNE on sequence embeddings with cosine kernel.
    vnes_tokens : list of float
        VNE on token embeddings with cosine kernel.
    vnes_multpl_combined : list of float
        Experimental. VNE using multiplicative combination of sequence and token kernels.
    vnes_add_combined : list of float
        Experimental. VNE using additive combination of sequence and token kernels.
    vnes_rbf : list of float
        VNE on sequence embeddings with RBF kernel.
    stds : list of float
        Standard deviation of the sequence embedding kernel matrix at each step.
    """
    vnes = []
    vnes_tokens = []
    vnes_add_combined = []
    ...
    for s_index, s in enumerate(seq_tokens): 
        if task_type != 'qa': 
            ...
            ems = emb_model.encode([question + ' ' + s for s in last_sentences], normalize_embeddings=True)
        else: 
            ems = emb_model.encode([question + ' ' + s for s in s['alternative_sequence_decoded']], normalize_embeddings=True)
        
        ems_token = emb_model.encode(s['alternative_tokens_str'], normalize_embeddings=True)  
        # ---- EXPERIMENTAL ---- FOR MORE EFFICIENT COMPUTATION
...
def uq_pipe_across_words(seq_words, emb_model, mode = 'sampling', question = "", task_type = 'qa'):
    """
    Computes kernel-based Von Neumann Entropy (VNE) uncertainty across words.
    Parameters and return values equal to uq_pipe_across_tokens()
    """
    vnes = []
    vnes_combined = []
    vnes_add_combined = []
...
def main(args): 
    """
    Computes token- and word-level uncertainty measures for pre-generated LLM answers.

    For each generation, produces two sets of uncertainty estimates (probability-based sampling, top-N selection-based). 
    Each set includes:

    - Semantic Entropy (SE): original and claim-conditional, at token and word level (duplicate across the two sets).
    - Kernel-based Von Neumann Entropy (VNE): under multiple kernel configurations (cosine, RBF,
      additive/multiplicative combinations), computed with both a contradiction- and a dispersion-focused embedding model, at token and word level.
    - Nep. log-probabilities and per-step entropies for both token and word subsequences.

    Parameters
    ----------
    args : argparse.Namespace
        Parsed command-line arguments. Expected fields:
        - model_id (str): HuggingFace model ID of the generation LLM.
        - emb_model_id (str): SentenceTransformer model ID for contradiction-focused embeddings.
        - emb_model_id_deltas (str): SentenceTransformer model ID for dispersion-focused embeddings.
        - ellm_model_id (str): Model ID for the entailment LLM (NLI or OpenAI).
        - exp_name (str): Experiment name prefix used in filenames.
        - dataset (str): Dataset identifier used to locate the generations file.
        - task_type (str): Task type ('qa' or other); affects embedding strategy.
        - n (int): Number of tokens to sample/select per step
    """
    ...
    for e, element in enumerate(generations):
        ...
        seq_tokens_sampled = generate_subsequences(step_sequences, llm.tokenizer, gen_ids, sampling_k = n, scaling_p = None, selection_p = 0.95, method = "sampling", question = question)
        ...
        seq_tokens_selection = generate_subsequences(step_sequences, llm.tokenizer, gen_ids, sampling_k = n, scaling_p = None, selection_p = 0.95, method = "top_k", question = question)
